...
class Summarizer:
    ...
    def get_last_entry_from_db(self, table_name, model):
        ...
        if model == "gpt-3.5-turbo-1106":
            ...
            print("Sufixo do modelo não reconhecido")

        try:
            conn = sqlite3.connect(f'databases/{self.nome_banco}')
            cursor = conn.cursor()
    
            # Define a coluna com base no modelo GPT fornecido
            column_name = f"resume_{model_sufix}"
